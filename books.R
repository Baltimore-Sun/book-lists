...
bookstbls <- bookstbls[c(11,1,4,8,6,2,3)]
...
###***CHECK WHETHER DATA ROWS COME IN SEMANTIC ORDER AND IF NOT ORDER HERE

#fiction
fiction <- bookstbls %>% filter(group == "Fiction")
write.csv(fiction[c(1:5)], "fiction.csv", row.names = FALSE)

#non-fiction: bio and auto
biography <- bookstbls %>% filter(subgroup == "Biography & Autobiography")
write.csv(biography[c(1:5)], "biography.csv", row.names = FALSE)

#non-fiction: self-help
selfhelp <- bookstbls %>% filter(subgroup == "Self-Help & Advice")
write.csv(selfhelp[c(1:5)], "selfhelp.csv", row.names = FALSE)

#non-fiction: lifestyle
lifestyle <- bookstbls %>% filter(subgroup == "Lifestyle")
write.csv(lifestyle[c(1:5)], "lifestyle.csv", row.names = FALSE)

#left, right and center
lrc <- bookstbls %>% filter(group == "Left, Right, and Center")
write.csv(lrc[c(1:5)], "lrc.csv", row.names = FALSE)

#children's
children <- bookstbls %>% filter(group == "Children's")
write.csv(children[c(1:5)], "children.csv", row.names = FALSE)

#young adult
ya <- bookstbls %>% filter(group == "Young Adult")
write.csv(ya[c(1:5)], "ya.csv", row.names = FALSE)

#recommended 
recs <- bookstbls %>% filter(group == "Recommended Books" & is.na(subgroup))
